...
class Net(nn.Module):
    ...
    def __init__(self, num_channels=256, num_res_blocks=7):
        ...
        # 初始化特征
        self.conv_block = nn.Conv2d(in_channels=10, out_channels=num_channels, kernel_size=(3, 3), stride=(1, 1), padding=1)
        ...
        self.policy_fc = nn.Linear(16 * 4 * 8, 352)
        # 价值头
    ...
    def forward(self, x):
        ...
        policy = torch.reshape(policy, [-1, 16 * 8 * 4])
        ...
        value = torch.reshape(value, [-1, 8 * 8 * 4])
...
class PolicyValueNet:
    ...
    def __init__(self, model_file=None, use_gpu=True):
        ...
        if model_file:
            self.policy_value_net.load_state_dict(torch.load(model_file, map_location=torch.device('cpu')))  # 加载模型参数
    ...
    def policy_value_fn(self, board):
        ...
        current_state = np.ascontiguousarray(board.current_state().reshape(-1, 10, 8, 4)).astype('float16')
        current_state = torch.as_tensor(current_state, dtype=torch.half, device=self.device)
        # 使用神经网络进行预测
...
if __name__ == '__main__':
    net = Net().to('cpu')
    test_data = torch.ones([8, 10, 8, 4]).to('cpu')
